...
namespace Utime_WEB.Controllers
{
    public class UserAuthController : Controller
    {
        ...
        {
            var response1 = await _service.LoginAsync<Response>(request);//запрос к методу LoginAsync
            if (response1 != null && response1.IsSuccess == true)
            {
                ...
                var token = handler.ReadJwtToken(response.Token);// прочтение токена
                identity.AddClaim(new Claim(ClaimTypes.Name, token.Claims.FirstOrDefault(u => u.Type == "name").Value));//получение имени 
                identity.AddClaim(new Claim(ClaimTypes.Role, token.Claims.FirstOrDefault(u => u.Type == "role").Value));//получения роли 
                principal.AddIdentity(identity);
                await HttpContext.SignInAsync(CookieAuthenticationDefaults.AuthenticationScheme, principal);//создание аутентификационных cookies
                HttpContext.Session.SetString("JWT-Token", response.Token);
                HttpContext.Response.Cookies.Append(".AspNetCore.JWT", response.Token, new CookieOptions { MaxAge = TimeSpan.FromMinutes(60) });// добавление jwt токена в cookies
